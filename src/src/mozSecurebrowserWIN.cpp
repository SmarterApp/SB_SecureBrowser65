...
static void __stdcall SBSpeechCallBack(WPARAM wParam, LPARAM lParam)
{
  ...
  nsresult rv = observerService->NotifyObservers(nsnull, "sb-word-speak", msg.get());
}
...
{
	// PrintLogMsg2("mozSecurebrowser::GetRunningProcessList"); 
	...
	{
		// PrintLogMsg2("CreateToolhelp32Snapshot (of processes) failed");
	} else 
	{
		...
		{
			// PrintLogMsg2("Process32First Failed!!!"); // show cause of failure
			CloseHandle( hProcessSnap );          // clean the snapshot object		
		} else 
		{
			HANDLE currentProcToken;
			PSID currentProcessSID;
			BOOL OKTOCOMPARE = false;
			if (OpenProcessToken( GetCurrentProcess(), TOKEN_QUERY, &currentProcToken )) 
			{	
				if(GetLogonSID(currentProcToken, &currentProcessSID))
				{
					OKTOCOMPARE = true;
				}
			}
			...
			do
			{
				...
				if (hProcess)
				{
					...
					{	
						PSID processSID;
						if(GetLogonSID(hToken, &processSID))
						{
							if(EqualSid(processSID, currentProcessSID))
							{
					...
					{
						PrintLogMsg2( "OpenProcessToken Error \n");
					}
				} 
/********
                                  else 
				{
					PrintLogMsg2( "OpenProcess Error \n");
				}
********/
			...
			if(OKTOCOMPARE)
			{
				HeapFree(GetProcessHeap(), 0, (LPVOID)currentProcessSID);
			}
...
{
  // PrintLogMsg2("mozSecurebrowser::Initialize"); 
...
{
  // PrintLogMsg2("mozSecurebrowser::Play"); 
...
{
  // PrintLogMsg2("mozSecurebrowser::Stop"); 
...
{
  // PrintLogMsg2("mozSecurebrowser::Pause"); 
...
{
  // PrintLogMsg2("mozSecurebrowser::Resume"); 
...
{
  // PrintLogMsg2("mozSecurebrowser::GetVoices"); 
...
{
  // PrintLogMsg2("mozSecurebrowser::GetVoice"); 
...
{
  // PrintLogMsg2("mozSecurebrowser::GetVolume"); 
...
{
  // PrintLogMsg2("mozSecurebrowser::GetStatus"); 
...
mozSecurebrowser::GetRate(PRInt32 *aRate)
{
  ...
  // PrintLogMsg2("mozSecurebrowser:GetRate"); 
...
mozSecurebrowser::SetRate(PRInt32 aRate)
{
  ...
  // PrintLogMsg2("mozSecurebrowser:SetRate"); 
...
{
  // PrintLogMsg2("mozSecurebrowser:GetSystemVolume"); 
  ...
  if (IsVistaOrGreater())
  {
    ...
    if (pEnumerator)
    {
      ...
      if (pDevice)
      {
        ...
        /********
        nsAutoString outNum, num;
        num.AppendFloat(currentVolume*10);
        PrintLogMsg2("GetMasterVolumeLevelScalar");
        PrintLogMsg2(num);
        ********/

        *aVolume = (int)ceil(currentVolume*10);

        /********
        PrintLogMsg2("GetSystemVolume RETURN VALUE");
        outNum.AppendInt(*aVolume);
        PrintLogMsg2(outNum);
        ********/
      }
    }
